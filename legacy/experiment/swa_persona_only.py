...
**Model (mặc định):** ``Qwen/Qwen2.5-7B-Instruct`` — Hugging Face ``AutoModelForCausalLM``,
**bf16**, **không** 4-bit; cùng cách load với ``exp_paper/exp_paper_hf_qwen25_7b_bf16.py``
(EXP-24-HF_QWEN25_7B_BF16). VRAM khoảng 15–18 GB bf16 trên GPU đủ lớn.

Ghi đè Hub id / short name: ``SWA_PERSONA_ONLY_MODEL``, ``SWA_PERSONA_ONLY_SHORT``.
Kaggle: thêm secret ``HF_TOKEN`` nếu cần tải trọng số.
...
_QWEN_HUB_DEFAULT = "Qwen/Qwen2.5-7B-Instruct"
...
def _default_swa_persona_model() -> str:
    ...
    if v:
        return v
    return _QWEN_HUB_DEFAULT


MODEL_NAME = _default_swa_persona_model()
MODEL_SHORT = (
    os.environ.get("SWA_PERSONA_ONLY_SHORT", "hf_qwen25_7b_bf16").strip()
    or "hf_qwen25_7b_bf16"
)
...
RESULTS_BASE = os.environ.get(
    ...
    (
        "/kaggle/working/cultural_alignment/results/exp_qwen25_swa_persona_only"
        if _on_kaggle()
        else str(_LOCAL_ROOT / "results" / "exp_qwen25_swa_persona_only")
    ),
)
...
def _load_swa_persona_model() -> tuple:
    """Qwen2.5-7B-Instruct: HF native, bf16, no 4-bit (parity with EXP-24 paper loader)."""
