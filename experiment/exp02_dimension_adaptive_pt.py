#!/usr/bin/env python3
"""
EXP-02: Dimension-Adaptive PT + Stratified Hierarchical IS
============================================================

**Why EXP-01 (SHIS-CG) is not enough:**

EXP-01's stratified prior and confidence gating improve country-level consistency,
but they cannot fix the ROOT CAUSE of the SocialValue bottleneck: the Prospect
Theory parameters (kappa=2.25, alpha=beta=0.88) are FIXED across all 6 moral
dimensions. This means the IS update treats Species decisions (where the anchor
is good) and SocialValue decisions (where the anchor is -27pp wrong) identically.

The high kappa=2.25 loss aversion penalizes ANY candidate that moves away from
the wrong SocialValue anchor, trapping the IS in a local minimum.

**EXP-02 Fix: Dimension-Specific PT Parameters**

    v_d(x) = { x^alpha_d        if x >= 0
             { -kappa_d |x|^beta_d  if x < 0

Calibrated from EXP-01 per-dimension error analysis:
  - SocialValue: anchor systematically wrong  -> kappa=1.25 (low loss aversion)
  - Species:     anchor generally correct     -> kappa=3.00 (high, prevent over-correction)
  - Age:         moderate-poor anchor         -> kappa=1.75
  - Others:      moderate anchor              -> kappa=2.00-2.25

Additionally: per-dimension sigma scaling (wider proposals where anchor is wrong).

**Combined with EXP-01:**
  - Category-stratified hierarchical prior
  - Confidence-gated prior application
  - ESS-adaptive anchor regularization

**Expected results:**
  - SocialValue gap reduced by 10-15pp (kappa 2.25 -> 1.25 allows 2x larger corrections)
  - Species/Gender stable (high kappa prevents drift)
  - Cross-model improvement (the PT fix is model-agnostic)
  - Cross-language improvement (the PT fix is language-agnostic)

Usage on Kaggle
---------------
    !python experiment/exp02_dimension_adaptive_pt.py
"""
...
# Stratified Hierarchical IS hyperparameters (from EXP-01)
N_WARMUP            = 30
DECAY_TAU           = 120
BETA_EMA            = 0.1
BETA_CAT            = 0.15
...
# Paths
BASE_ROOT = "/kaggle/working/cultural_alignment/results/exp02_baseline"
SWA_ROOT  = "/kaggle/working/cultural_alignment/results/exp02_dim_pt/swa"
CMP_ROOT  = "/kaggle/working/cultural_alignment/results/exp02_dim_pt/compare"
...
# ============================================================================
# Step 4: Stratified Prior State (reused from EXP-01)
# ============================================================================
class StratifiedPriorState:
    ...
    def update(self, delta_micro: float, category: str):
        self.step += 1
        ...
        self.cat_priors[category] = (
            (1.0 - BETA_CAT) * self.cat_priors[category] + BETA_CAT * delta_micro
        )
...
# ============================================================================
# Step 5: Dimension-Adaptive Controller
# ============================================================================
class Exp02DimAdaptiveController(ImplicitSWAController):
    """
    Dimension-Adaptive PT + Stratified Hierarchical IS.

    Two key innovations over EXP-01:
      1. Dimension-specific kappa/alpha/beta/sigma_scale in PT value function
      2. Dimension-scaled proposal width (wider for SocialValue, narrower for Species)

    Inherited from EXP-01:
      3. Category-stratified hierarchical prior
      4. Confidence-gated prior application
      5. ESS-adaptive anchor regularization
    """
    ...
    def predict(self, user_query, preferred_on_right=True,
        ...
        # ── K-sample IS with DIMENSION-SPECIFIC PT utility ──
        ...
        # ── Stratified Prior + Confidence Gating (from EXP-01) ──
        ...
        # ── ESS-Adaptive Anchor Regularization (from EXP-01) ──
        ...
        return {
            ...
            "kappa_d": kappa_d, "sigma_scale": sigma_scale,
...
_swa_runner_mod.ImplicitSWAController = Exp02DimAdaptiveController
...
def _run_swa_for_model(model, tokenizer, model_name):
    ...
    out_dir.mkdir(parents=True, exist_ok=True); cfg.output_dir = str(out_dir)
    print(f"\n{'#'*70}\n# {EXP_ID} DIM-ADAPTIVE PT [{model_name}]\n{'#'*70}")
    rows = []
    for ci, country in enumerate(cfg.target_countries):
        if country not in SUPPORTED_COUNTRIES: continue
...
def _print_comparison_table(cmp_df):
    ...
    for model_name in cmp_df["model"].drop_duplicates().tolist():
        ...
        print(f"\n{'='*width}")
        print(f"  {EXP_ID} DIM-ADAPTIVE PT  |  MIS Comparison  |  lower = better")
        print(f"  MODEL: {short}")
        print(f"{'='*width}")
...
def main():
    ...
    print(f"\n[{EXP_ID}] Dimension-Adaptive PT + Stratified Hierarchical IS")
    print(f"[CONFIG] PT params per dimension:")
    for dim, p in DIMENSION_PT_PARAMS.items():
        print(f"  {dim:>15}: kappa={p['kappa']:.2f}  sigma_scale={p['sigma_scale']:.1f}")
