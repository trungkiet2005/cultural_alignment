...
Estimates a static per-country (b_a, b_b) decision-token bias from a
small WVS profile corpus and adds it to the decision-token logits at
inference. Strict contrast to SWA-DPBR's per-scenario persona signal:
SWAI's correction is *constant per country* and uses no within-country
disagreement information.

See ``src/swai_baseline.py`` for the implementation.

Kaggle (~1-1.5h on H100 for Phi-4 x 20 countries):
    !python exp_paper/review/round3/baselines/exp_r3_baseline_swai.py
...
from src.swai_baseline import run_swai_country  # noqa: E402
...
def main() -> None:
    ...
    for country in COUNTRIES:
        ...
        try:
            ...
            summary = out["summary"]
            rows.append(summary)
            out["results_df"].to_csv(out_dir / f"swai_results_{country}.csv", index=False)
            pd.DataFrame(rows).to_csv(out_dir / "swai_partial.csv", index=False)
            ...
            rows.append({"method": "swai", "country": country,
                         "error": str(exc)[:500]})
        gc.collect()
